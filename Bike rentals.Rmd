...
```{r scatter actual temp against number of casual bikers}

ggplot(bikedata, aes(actual.temp, casual)) + geom_point(shape=1) + geom_abline(intercept = coef(casual_temp.regress)[1], slope = coef(casual_temp.regress)[2], colour = "red") + ylab('Casual bike rental') + xlab('actual temperature') + labs(title = 'temperature against  casual rental count (model imposed)')

```

```{r scatter actual temp against number of registered bikers}

ggplot(bikedata, aes(actual.temp, registered)) + geom_point(shape=1) + geom_abline(intercept = coef(registered_temp.regress)[1], slope = coef(registered_temp.regress)[2], colour = "red") + ylab('registered bike rental') + xlab('actual temperature') + labs(title = 'temperature against  registered rental count (model imposed)')
...
```{r scatter feel temp against number of casual bikers}

ggplot(bikedata, aes(actual.atemp, casual)) + geom_point(shape=1) + geom_abline(intercept = coef(casual_feeltemp.regress)[1], slope = coef(casual_feeltemp.regress)[2], colour = "red") + ylab('Casual bike rental') + xlab('feel temperature') + labs(title = 'feel temperature against  casual rental count (model imposed)')
...
ggplot(bikedata, aes(actual.atemp, registered)) + geom_point(shape=1) + geom_abline(intercept = coef(registered_feeltemp.regress)[1], slope = coef(registered_feeltemp.regress)[2], colour = "red") + ylab('registered bike rental') + xlab('feel temperature') + labs(title = 'feel temperature against  registered rental count (model imposed)')

```

```{r difference between actual temperature and feel temperature}
bikedata <- 
  bikedata %>% mutate(diff_temp = actual.temp - actual.atemp)
